...
//bool MenuScene::keyIsPressed(KeyType keyCode) {
...
void StartScene::rotate(bool clockwise) {
	...
	std::vector<cocos2d::Vector<FiniteTimeAction*>> vec(m_characterCount);

	int rotations = 360 / m_characterCount;
	for (int i = 0; i < rotations; ++i) {
		for (int j = 0; j < m_characterCount; ++j) {
			polarToCartesian(m_degrees[j], x, y);
			vec[j].pushBack(cocos2d::MoveTo::create(0.001f, cocos2d::Vec2(x, y)));
			setAngle(m_degrees[j] += scalingFactor);
		}
	}

	const static float largeScaling = 2.5f;
	const static float smallScaling = 1.0f;

	std::vector<cocos2d::Sequence*> seq(m_characterCount);
	for (int i = 0; i < m_characterCount; ++i)
		seq[i] = cocos2d::Sequence::create(vec[i]);

	for (int i = 0; i < m_characterCount; ++i) {
		std::string character = "p" + std::to_string(i + 1);
		sprites.find(character)->second->runAction(seq[i]);
		sprites.find(character)->second->runAction(cocos2d::ScaleTo::create(0.1f, index == i ? largeScaling : smallScaling));
	}
...
void StartScene::setCharacterSelectPositions() {
	int start_degree = 270;
	m_degrees.push_back(start_degree);
	for (int i = 1; i < m_characterCount; ++i)
		m_degrees.push_back(start_degree += 360 / m_characterCount);
}
...
void LevelScene::LevelKeyPressed(KeyType keyCode, Event* event) {
	...
	if (m_currentDungeon->getPlayer()->hasWon()) {
		m_hud->winner();
		return; // prevents timer from being scheduled
	}
...
cocos2d::Scene* LevelEditor::createScene() {
	return LevelEditor::create();
}
