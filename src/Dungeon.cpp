...
void Dungeon::trapEncounter(int x, int y, Coords &playerCoords) {
	std::vector<int> indexes = findTraps(x, y);
	if (playerCoords == Coords())
		playerCoords = Coords(x, y);

	for (size_t i = 0; i < indexes.size(); ++i) {
		m_traps.at(indexes[i])->trapActionCheck(*getPlayerAt(playerCoords));
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			...
			case 'A': { 
				...
				addTrap(std::make_shared<Ledge>(*this, j, i));
