...
void EtherealSpell::useItem(Dungeon &dungeon, Player &player) {
	...
	player.addAffliction(std::make_shared<Ethereality>(&dungeon, 15 + 2 * player.getInt()));
...
Ledge::Ledge(Dungeon &dungeon, int x, int y, char facing) : Traps(&dungeon, x, y, LEDGE, "Cheese_Wedge_48x48.png", 0) {
	dungeon.tileAt(getPos()).wall = true;
	...
	if (facing != '-') {
		...
		switch (randInt(1, 4)) {
		case 1: m_facing = 'l'; break;
		case 2: m_facing = 'r'; break;
		case 3: m_facing = 'u'; break;
		case 4: m_facing = 'd'; break;
		}
	}
}

void Ledge::trapAction(Actors &a) {
	char actorAction;

	if (a.isPlayer()) {
		actorAction = m_dungeon->getPlayer()->getAction();
		if (!isMovementAction(actorAction))
			return;
	}
	else {
		actorAction = getCardinalFacingDirectionRelativeTo(a.getPos(), getPos());
	}

	if (directionIsOppositeTo(m_facing, actorAction)) {
		return;
	}
	
	a.moveTo(getPos().x, getPos().y);	
}
...
DirtMound::DirtMound(Dungeon &dungeon, int x, int y) : Traps(&dungeon, x, y, COMBUSTION, "Emerald_48x48.png", 0) {
	setSprite(dungeon.createSprite(getPos(), getHalfTileYOffset(), y + Z_WALL, getImageName()));
